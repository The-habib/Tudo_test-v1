...
const ICON_OPTIONS = ["Target","Trophy","Star","Rocket","BookOpen","Droplets","Leaf","Heart","Zap","Flame","Music","Globe"];
...
export default function Goals() {
  ...
  async function handleAdd() {
    ...
    await addGoal.mutateAsync({ title: title.trim(), icon });
    setTitle(""); setIcon("Target"); setShowModal(false);
  }
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-sm font-black text-[#1a1a1a]">No goals yet</p>
          <p className="text-xs text-gray-400 text-center">Set a goal and start working<br />towards your dreams</p>
        ...
        <GoalGrid goals={goals} onToggle={(id) => toggleGoal.mutate(id)} onDelete={(id) => deleteGoal.mutate(id)} />
      )}
            ...
            <motion.div initial={{ opacity: 0, y: 80, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
              ...
              <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-base font-black text-[#1a1a1a]">New Goal</h2>
                  <button onClick={() => setShowModal(false)}
                    className="w-8 h-8 rounded-xl bg-[#1a1a1a]/5 flex items-center justify-center">
                    <X className="w-4 h-4 text-[#1a1a1a]" />
                  </button>
                </div>
                <input type="text" placeholder="e.g. Read 12 books this year"
                ...
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleAdd}
                  ...
                  {addGoal.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                  Add Goal
                </motion.button>
              </div>
            </motion.div>
