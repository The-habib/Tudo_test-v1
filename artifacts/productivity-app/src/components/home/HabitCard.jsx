...
import { Flame, Leaf, PersonStanding, BookOpen } from "lucide-react";

const habits = [
  { icon: Leaf, label: "Yoga" },
  { icon: PersonStanding, label: "Run" },
  ...
  return (
    ...
    >
              ...
              return (
                <motion.div
                  ...
                  className={`w-11 h-11 rounded-2xl flex items-center justify-center shadow-sm ${
                    i === 1 ? "bg-[#C5D86D]" : "bg-[#1a1a1a]"
                  }`}
                >
                  <Icon className={`w-5 h-5 ${i === 1 ? "text-[#1a1a1a]" : "text-[#C5D86D]"}`} strokeWidth={2} />
                </motion.div>
              );
            })}
          </div>
          <p className="text-[10px] text-gray-400 mt-3 font-medium">3 active today</p>
        </motion.div>
      </Link>
      ...
      >
        ...
        <span className="text-[9px] text-gray-400 mt-1 font-medium text-center leading-tight">
          Streak Days
        </span>
      </motion.div>
    </motion.div>
