...
export default function GoalStats({ goals = [] }) {
  const finished = goals.filter((g) => g.completed).length;
  const doing = goals.filter((g) => !g.completed && !g.abandoned).length;
  const abandoned = goals.filter((g) => g.abandoned).length;

  const stats = [
    { count: finished, label: "Finished", color: "text-[#C5D86D]", bg: "bg-[#C5D86D]/10" },
    { count: doing, label: "Doing", color: "text-white", bg: "bg-white/10" },
    { count: abandoned, label: "Abandon", color: "text-gray-400", bg: "bg-white/5" },
  ];
      ...
      <div className="flex gap-3">
        {stats.map((stat, i) => (
          <motion.div key={i} initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <motion.p key={stat.count} initial={{ scale: 0.7 }} animate={{ scale: 1 }}
              className={`text-3xl font-black leading-none ${stat.color}`}>{stat.count}</motion.p>
